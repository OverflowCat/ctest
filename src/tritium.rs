...
pub const BIT_COLD_TRAP_TEMP: u16 = 1 << 2;
...
pub const BIT_ABS_HUMIDITY: u16 = 1 << 10;
...
pub struct TritiumInstantData {
    pub env_temperature_c: Option<f32>,
    pub cold_trap_temperature_c: Option<f32>,
    pub instant_flow_a_lpm: Option<f32>,
    pub instant_flow_b_lpm: Option<f32>,
    pub env_humidity_pct: Option<f32>,
    pub pressure_kpa: Option<f32>,
    pub standard_volume_a_l: Option<f32>,
    pub standard_volume_b_l: Option<f32>,
    pub standard_mode_c: Option<i16>,
    pub working_volume_a_l: Option<f32>,
    pub working_volume_b_l: Option<f32>,
    pub abs_humidity_g_m3: Option<f32>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TritiumHistoryRecord {
    pub index: u16,
    pub start_time: DeviceDateTime,
    pub end_time: DeviceDateTime,
    pub avg_flow_a_lpm: f32,
    pub avg_flow_b_lpm: f32,
    pub standard_volume_a_l: f32,
    pub standard_volume_b_l: f32,
    pub cold_trap_temperature_c: f32,
    pub pressure_kpa: f32,
    pub standard_mode_c: i16,
    pub set_flow_lpm: f32,
    pub working_volume_a_l: f32,
    pub working_volume_b_l: f32,
    pub env_temperature_c: f32,
}

pub struct TritiumSampler {
...
impl TritiumSampler {
    ...
    pub async fn read_history(&mut self, index: u16) -> Result<TritiumHistoryRecord> {
    ...
    pub async fn query_instant(&mut self, mask: u16) -> Result<TritiumInstantData> {
    ...
    pub async fn set_cold_trap_temp(&mut self, temp_c: f32) -> Result<()> {
        if !(-20.0..=0.0).contains(&temp_c) {
            return Err(anyhow!("cold trap temperature out of range [-20,0]: {}", temp_c));
        }
        let payload = self.transport.send_recv(0x22, &temp_c.to_be_bytes()).await?;
        check_response_ok(&payload, 0x23, "set_cold_trap_temp")
    }

    pub async fn set_defrost_time(&mut self, hours: u8, minutes: u8) -> Result<()> {
        if hours > 9 || minutes > 59 {
            return Err(anyhow!("invalid defrost time {}h{}m", hours, minutes));
        }
        let payload = self.transport.send_recv(0x24, &[hours, minutes]).await?;
        check_response_ok(&payload, 0x25, "set_defrost_time")
    }
...
fn parse_history_record(index: u16, d: &[u8]) -> Result<TritiumHistoryRecord> {
    if d.len() < 73 {
        return Err(anyhow!("history record too short: {}", d.len()));
    }
    Ok(TritiumHistoryRecord {
        index,
        start_time: DeviceDateTime::from_bytes(&d[19..24]),
        end_time: DeviceDateTime::from_bytes(&d[24..29]),
        avg_flow_a_lpm: parse_f32_be(&d[29..33]),
        avg_flow_b_lpm: parse_f32_be(&d[33..37]),
        standard_volume_a_l: parse_f32_be(&d[37..41]),
        standard_volume_b_l: parse_f32_be(&d[41..45]),
        cold_trap_temperature_c: parse_f32_be(&d[45..49]),
        pressure_kpa: parse_f32_be(&d[51..55]),
        standard_mode_c: i16::from_be_bytes([d[55], d[56]]),
        set_flow_lpm: parse_f32_be(&d[57..61]),
        working_volume_a_l: parse_f32_be(&d[61..65]),
        working_volume_b_l: parse_f32_be(&d[65..69]),
        env_temperature_c: parse_f32_be(&d[69..73]),
    })
}

fn parse_instant_data(mask: u16, data: &[u8]) -> Result<TritiumInstantData> {
    let mut pos = 0usize;
    let mut out = TritiumInstantData {
        env_temperature_c: None,
        cold_trap_temperature_c: None,
        instant_flow_a_lpm: None,
        instant_flow_b_lpm: None,
        env_humidity_pct: None,
        pressure_kpa: None,
        standard_volume_a_l: None,
        standard_volume_b_l: None,
        standard_mode_c: None,
        working_volume_a_l: None,
        working_volume_b_l: None,
    ...
    let take_i16 = |buf: &[u8], p: &mut usize| -> Result<i16> {
        if *p + 2 > buf.len() {
            return Err(anyhow!("instant payload truncated"));
        }
        let v = i16::from_be_bytes([buf[*p], buf[*p + 1]]);
        *p += 2;
        Ok(v)
    };
    ...
    if (mask & BIT_COLD_TRAP_TEMP) != 0 {
        out.cold_trap_temperature_c = Some(take_f32(data, &mut pos)?);
    }
    if (mask & BIT_INSTANT_FLOW) != 0 {
        out.instant_flow_a_lpm = Some(take_f32(data, &mut pos)?);
        out.instant_flow_b_lpm = Some(take_f32(data, &mut pos)?);
    }
    if (mask & (1 << 4)) != 0 {
        pos += 2;
    }
    ...
    if (mask & BIT_STANDARD_VOLUME) != 0 {
        out.standard_volume_a_l = Some(take_f32(data, &mut pos)?);
        out.standard_volume_b_l = Some(take_f32(data, &mut pos)?);
    }
    if (mask & BIT_STANDARD_MODE) != 0 {
        out.standard_mode_c = Some(take_i16(data, &mut pos)?);
    }
    if (mask & BIT_WORKING_VOLUME) != 0 {
        out.working_volume_a_l = Some(take_f32(data, &mut pos)?);
        out.working_volume_b_l = Some(take_f32(data, &mut pos)?);
    }
    if (mask & BIT_ABS_HUMIDITY) != 0 {
        out.abs_humidity_g_m3 = Some(take_f32(data, &mut pos)?);
    }
